...
/* ---- SOH per-customer labelling helpers ---- */
function sohCustomerValueFor(rec) {
  if (rec && rec.customer_id) return 'cust:' + rec.customer_id;
  if (rec && rec.customer_label) return 'label:' + rec.customer_label;
  return '';
}
function parseSohCustomerValue(val) {
  if (!val) return { customer_id: null, customer_label: null };
  if (val.startsWith('cust:')) {
    const id = val.slice(5);
    return { customer_id: id, customer_label: State.customers.find(c => c.id === id)?.name || null };
  }
  if (val.startsWith('label:')) return { customer_id: null, customer_label: val.slice(6) };
  return { customer_id: null, customer_label: null };
}
...
function sohCustomerOptionsHtml(selectedValue, { includeNone = true, noneLabel = '— None —' } = {}) {
  ...
  if (selectedValue && !known.has(selectedValue)) {
    const txt = selectedValue.startsWith('label:') ? selectedValue.slice(6)
      : selectedValue.startsWith('cust:') ? '(removed customer)' : selectedValue;
    orphan = `<option value="${esc(selectedValue)}" selected>${esc(txt)}</option>`;
  }
  return `
...
async function renderStockReport(content) {
  ...
  content.innerHTML = `
    ...
    <div class="table-wrap" style="margin-bottom:24px;">
      <table>
        <thead><tr><th>Date</th><th>Type</th><th class="num">Pallets</th><th class="num">Cans (M)</th><th>Customer</th><th>Description</th><th>By</th></tr></thead>
        <tbody>
          ${inPeriod.length ? inPeriod.slice().sort((a, b) => a.movement_date < b.movement_date ? 1 : -1).map(m => `
            <tr>
              ...
              <td><select class="soh-relabel" data-mtype="movement" data-id="${m.id}">${sohCustomerOptionsHtml(sohCustomerValueFor(m), { noneLabel: '— Unassigned —' })}</select></td>
              <td class="small muted">${esc(m.description || '')}</td>
              <td class="small muted">${esc(m.created_by_email || (m.movement_type === 'dispatch' ? 'auto' : ''))}</td>
            </tr>`).join('') : `<tr><td colspan="7" class="empty-state">No stock movements in this period.</td></tr>`}
        </tbody>
      </table>
    </div>
  ...
  content.querySelectorAll('.soh-relabel').forEach(sel => sel.addEventListener('change', async () => {
    ...
    try {
      if (sel.dataset.mtype === 'design') await DB.updateSohDesignRecord(sel.dataset.id, { customer_id, customer_label });
      else await DB.updateSohMovement(sel.dataset.id, { customer_id, customer_label });
      toast('Customer label updated', 'ok');
      renderContent();
    } catch (err) { toast(err.message, 'err'); }
  }));
}
...
function openReceiptModal(kind) {
  ...
  $('#modal-save').addEventListener('click', async () => {
    const pallets = $('#f-pallets').value;
    const custVal = $('#f-customer').value;
    if (!custVal) { toast('Choose the customer this stock is for', 'err'); return; }
    if (!pallets) { toast('Enter a pallet quantity', 'err'); return; }
    const { customer_id, customer_label } = parseSohCustomerValue(custVal);
    const stamp = currentUserStamp();
    try {
